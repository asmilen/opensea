...
system.controller("TicketComponentController", AttractionController);
/**
 *
 * @param {type} $scope
 * @param {type} $http
...
function AttractionController($scope, $http, $rootScope, $timeout, Upload) {
    $scope.controllerName = "TicketComponentController";
    $scope.pageId = 0;
    $scope.filter = {
        key: ""
    };
    ...
    $scope.find = function(){
        ...
        url = api_domain + "/api/ticket-component/index";
        $http({
            url: url,
            method: "GET",
            params: param,
    ...
    $scope.createArticle = function () {
        ...
        $http.post(api_domain + "/api/ticket-component/create", param).success(function (data) {
    ...
    $scope.updateArticle = function () {
        ...
        $http.post(api_domain + "/api/ticket-component/update", param).success(function (data) {
            if (data.status == 'successful') {
    ...
    $scope.deleteArticle = function (article) {
        ...
        $http.post(api_domain + "/api/ticket-component/delete", {id: article.id, api_token: api_token}).success(function (data) {
            if (data.status != 'successful') {
                showMessage("Error", data.message, "error");
            } else {
