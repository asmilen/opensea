...
system.controller("TourRequestController", TourRequestController);
/**
 *
 * @param {type} $scope
 * @param {type} $http
 ...
 * @returns {undefined}
 */
function TourRequestController($scope, $http, $rootScope, $timeout, Upload) {
    $scope.controllerName = "TourRequestController";
    $scope.pageId = 0;
    $scope.filter = {
        key: ""
    };
    ...
    $scope.find = function(){
        ...
        if ($scope.filter.search_tour) {
            param.search_tour = $scope.filter.search_tour;
        }
        ...
        url = api_domain + "/api/tour-request/index";
        $http({
            url: url,
            method: "GET",
            params: param,
